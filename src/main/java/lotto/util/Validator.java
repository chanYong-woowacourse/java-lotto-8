...
public class Validator {
    ...
    private static final String NOT_NUMBER = "금액은 숫자여야 합니다.";
    private static final String NEGATIVE_AMOUNT = "금액은 음수가 될 수 없습니다.";
    private static final String INVALID_UNIT = "금액은 1000원 단위여야 합니다.";
    ...
    private static void validateNonNegative(int amount) {
        if (amount < 0) {
            throw new IllegalArgumentException(ERROR_PREFIX + NEGATIVE_AMOUNT);
        }
    }

    private static void validateThousandUnit(int amount) {
        if (amount % 1000 != 0) {
            throw new IllegalArgumentException(ERROR_PREFIX + INVALID_UNIT);
        }
    }
}
